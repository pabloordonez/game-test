...
export class CollisionSystem implements System {
	...
	private isEntityOffScreen(bounds: Bounds, buffer: number): boolean {
		return bounds.right < -buffer || bounds.left > this.viewport.width + buffer || bounds.bottom < -buffer || bounds.top > this.viewport.height + buffer;
	}
	...
	private shouldDropPowerUp(_: BlockComponent): boolean {
	...
	private spawnPowerUp(block: Entity): void {
		...
		PowerUp.create(this.world, position.x, position.y, randomType);
	}
