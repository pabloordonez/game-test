...
export class StarParticleSystem {
	...
	update(deltaTime: number): void {
		...
		for (let i = this.particles.length - 1; i >= 0; i--) {
			...
			const distanceFromCenter = Math.sqrt((particle.x - this.centerX) ** 2 + (particle.y - this.centerY) ** 2);

			// Calculate maximum distance to screen edge (with buffer)
			const maxDistance = Math.sqrt((this.screenWidth / 2) ** 2 + (this.screenHeight / 2) ** 2) * 1.5; // Even longer visibility
			...
			if (
				...
				particle.x < -buffer ||
				particle.x > this.screenWidth + buffer ||
				particle.y < -buffer ||
				particle.y > this.screenHeight + buffer
			) {
